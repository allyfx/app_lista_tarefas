	...
	<head>
		...
		<script>
			function editar(id, desc) {
				...
				form.action = 'index.php?acao=atualizar&page=index';
				form.method = 'post';
				form.className = 'row'
			...
			function remover(id) {
				location.href = 'index.php?acao=remover&id='+id+'&page=index';
			}

			function marcarRealizada(id) {
				location.href = 'index.php?acao=marcarRealizada&id='+id+'&page=index';
			}
		</script>
	</head>
								...
								<? foreach ($tarefas as $key => $value) { ?>
									...
									<? if($value->status == 'pendente') { ?>
										<div class="row mb-3 d-flex align-items-center tarefa">
											<div id="tarefa_<?= $value->id ?>" class="col-sm-9">
												<?= $value->tarefa ?>
											</div>
